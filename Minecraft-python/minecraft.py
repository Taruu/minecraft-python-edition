        ...
        for c in i:
            if c != "\n":
                if stage == 0:
                    ...
                    if c == "$":
                        mas[name] = [tex_coords(tuple(texCords[0]),tuple(texCords[1]),tuple(texCords[2])), atlas, texCords, tags]
                        name = ""
                        n1 = ""
                        n2 = ""
                        atlas = ""
...
class Model(object):
    ...
    def __init__(self):
        ...
        for i in files:
            if i[-1] == "l":
                self.atlases  = parseTexAtlas("mods/" + i, self.atlases)
            elif i[-1] == "s":
                self.blocks = parseBlocks("mods/" + i, self.blocks)
            ...
            for z in xrange(0, n, s):
                ...
                if (h > 20):
                    if random.randrange(0, 1000) > 990:
                        ...
                        # Tree trunk
                        GENERATABLETREES = [[self.blocks["WOOD"],self.blocks["BWOOD"]],[self.blocks["LEAF"],self.blocks["BLEAF"]]]
